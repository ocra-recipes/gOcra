 ...
 *
 * \brief Define contact avoidance constraint for ISIR controller.
 */
 ...
 *
 * To relate this constraint with the dynamic variables, we constrain the estimated future distances to remain positive, as explained in orcisir::JointLimitFunction .
 *
 ...
 * \f}
 *
 * There is some similarity with the orcisir::JointLimitFunction because we constrain an estimated future state. The same issues arise (it only constrain the final point, no middle points),
 ...
 *
 * See \ref orcisir::ContactAvoidanceFunction for more information.
 */
class FullContactAvoidanceFunction: public ContactAvoidanceFunction
{
    public:
        ...
        FullContactAvoidanceFunction(const orc::Model& model);
...
class ReducedContactAvoidanceFunction: public ContactAvoidanceFunction
{
    public:
        ...
        ReducedContactAvoidanceFunction(const orc::Model& model, const ISIRDynamicFunction& dynamicEquation);
...
class ContactAvoidanceConstraint: public ISIRConstraint
{
public:
    ContactAvoidanceConstraint(const orc::Model& model, double hpos, double margin);
...
private:
    ContactAvoidanceFunction* createFullContactAvoidanceFunction(const orc::Model& model);
    ContactAvoidanceFunction* createReducedContactAvoidanceFunction(const orc::Model& model, const ISIRDynamicFunction& dynamicEquation);
