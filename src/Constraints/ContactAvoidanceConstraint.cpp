 ...
 *
 * \brief Implement contact avoidance constraint for ISIR controller.
 */
 ...
 */
ContactAvoidanceFunction::ContactAvoidanceFunction(const orc::Model& model, orc::Variable& var)
    : orc::NamedInstance("ISIR Contact Avoidance Function")
...
//    , _q_ddot(model.getAccelerationVariable())
    ...
    , margin(.05)
{
    model.connect<orc::EVT_CHANGE_VALUE>(*this, &ContactAvoidanceFunction::invalidateAll);
    model.connect<orc::EVT_CHANGE_VALUE>(*this, &ContactAvoidanceFunction::invalidateb);
}
 ...
 * Does nothing.
 *
 * \todo It should be conform to the xde framework, the computation of the jacobian should be here.
 */
 ...
 * Does nothing.
 *
 * \todo It should be conform to the xde framework, the computation of the vector should be here.
 */
 ...
 */
FullContactAvoidanceFunction::FullContactAvoidanceFunction(const orc::Model& model)
    : orc::NamedInstance("Full Contact Avoidance Function")
 ...
 * \param model The xde model on which we will update the dynamic parameters
 */
ReducedContactAvoidanceFunction::ReducedContactAvoidanceFunction(const orc::Model& model, const ISIRDynamicFunction& dynamicEquation)
    : orc::NamedInstance("Reduced Contact Avoidance Function")
    , orc::AbilitySet(orc::PARTIAL_X)
    , orc::CoupledInputOutputSize(false)
...
ContactAvoidanceConstraint::ContactAvoidanceConstraint(const orc::Model& model, double hpos, double margin)
    : _model(model)
    , _is_connected(false)
    , ISIRConstraint()
{
...
void ContactAvoidanceConstraint::connectToController(const ISIRDynamicFunction& dynamicEquation, bool useReducedProblem)
{
...
ContactAvoidanceFunction* ContactAvoidanceConstraint::createFullContactAvoidanceFunction(const orc::Model& model)
{
    _contactAvoidanceFunction = new FullContactAvoidanceFunction(model);
    return _contactAvoidanceFunction;
}

ContactAvoidanceFunction* ContactAvoidanceConstraint::createReducedContactAvoidanceFunction(const orc::Model& model, const ISIRDynamicFunction& dynamicEquation)
{
