 ...
 *
 * \brief Implement torque limit constraint for ISIR controller.
 */
 ...
 *
 * It initialize the Jacobian of the linear function, \f$ \A \f$ which is constant.
 */
TorqueLimitFunction::TorqueLimitFunction(const orc::Model& model)
    : orc::NamedInstance("Torque Limit Equation Function")
    ...
    , LinearFunction(model.getJointTorqueVariable(), 2*model.getJointTorqueVariable().getSize())
{
    ...
    //_model.connect<orc::EVT_CHANGE_VALUE>(*this, &JointLimitFunction::invalidateb);   // b is constant, unless someone changes it by calling setTorqueLimits
