 ...
 *
 * \brief Implement joint limit constraint for ISIR controller.
 */
 ...
 *
 * \param model The xde model on which we will get the dynamic parameters
 ...
 */
JointLimitFunction::JointLimitFunction(const orc::Model& model, orc::Variable& var)
    : orc::NamedInstance("Joint Limit Equation Function")
 ...
 *
 * The computation of \f$ \A \f$ is explained in orcisir::JointLimitFunction
 */
 ...
 * and then transformed (see \ref sec_transform_formalism) to fit the new variable.
 *
 * The computation of \f$ \b \f$ is explained in orcisir::JointLimitFunction
 */
 ...
 *
 * It is connected with the model and invalidates \f$ \b \f$ when orc::EVT_CHANGE_VALUE is raised.
 * Furthermore, it computes the Jacobian matrix \f$ \A \f$ which is constant.
 */
FullJointLimitFunction::FullJointLimitFunction(const orc::Model& model)
    : orc::NamedInstance("Full Joint Limit Equation Function")
    ...
    , pimpl(new Pimpl(model))
{
    ...
    pimpl->_model.connect<orc::EVT_CHANGE_VALUE>(*this, &FullJointLimitFunction::invalidateb); //to update b at each time step
...
FullJointLimitFunction::~FullJointLimitFunction()
{
    ...
    pimpl->_model.disconnect<orc::EVT_CHANGE_VALUE>(*this, &FullJointLimitFunction::invalidateb);
}
...
{
    const orc::Model&          _model;
    const ISIRDynamicFunction& _dynamicEquation;

    Eigen::MatrixXd            _fullJacobian;

    Pimpl(const orc::Model& model, const ISIRDynamicFunction& dynamicEquation)
        : _model(model)
 ...
 * \param model The xde model on which we will update the dynamic parameters
 *
 * It is connected with the model and invalidates all when orc::EVT_CHANGE_VALUE is raised.
 ...
 */
ReducedJointLimitFunction::ReducedJointLimitFunction(const orc::Model& model, const ISIRDynamicFunction& dynamicEquation)
    : orc::NamedInstance("Reduced Joint Limit Equation Function")
    , orc::AbilitySet(orc::PARTIAL_X)
    , orc::CoupledInputOutputSize(false)
    , JointLimitFunction(model, dynamicEquation.getActionVariable())
    , pimpl(new Pimpl(model, dynamicEquation))
{
    pimpl->_model.connect<orc::EVT_CHANGE_VALUE>(*this, &ReducedJointLimitFunction::invalidateAll);
    pimpl->_model.connect<orc::EVT_CHANGE_VALUE>(*this, &ReducedJointLimitFunction::invalidateb); //to update b at each time step
...
{
    pimpl->_model.disconnect<orc::EVT_CHANGE_VALUE>(*this, &ReducedJointLimitFunction::invalidateAll);
    pimpl->_model.disconnect<orc::EVT_CHANGE_VALUE>(*this, &ReducedJointLimitFunction::invalidateb);
}
...
///** Get the action variable \f$ \av \f$ from the a ISIRModel instance.
// *
// * \param model The xde model on which we will update the dynamic parameters
// * \return The action variable \f$ \av = [ \torque \; \force_c ] \f$ from the \a model instance
// */
//orc::Variable& ReducedJointLimitFunction::getActionVariable(const orc::Model& model)
//{
//    const ISIRModel& mIM  = static_cast<const ISIRModel&>(model);
//
//    return mIM.getActionVariable();
//}
...
JointLimitConstraint::JointLimitConstraint(const orc::Model& model, double hpos)
    : _model(model)
    , _is_connected(false)
    , ISIRConstraint()
{
...
JointLimitConstraint::JointLimitConstraint(const orc::Model& model, const Eigen::VectorXd& lowerLimits, const Eigen::VectorXd& upperLimits, double hpos)
    : _model(model)
    , _is_connected(false)
    , ISIRConstraint()
{
...
void JointLimitConstraint::connectToController(const ISIRDynamicFunction& dynamicEquation, bool useReducedProblem)
{
...
JointLimitFunction* JointLimitConstraint::createFullJointLimitFunction(const orc::Model& model)
{
    _jointLimitFunction = new FullJointLimitFunction(model);
    return _jointLimitFunction;
}

JointLimitFunction* JointLimitConstraint::createReducedJointLimitFunction(const orc::Model& model, const ISIRDynamicFunction& dynamicEquation)
{
